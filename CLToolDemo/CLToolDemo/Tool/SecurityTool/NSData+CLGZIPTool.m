...
@implementation NSData (CLGZIPTool)
...
- (NSData *)cl_gzippedDataWithCompressionLevel:(float)level
{
    if (self.length == 0 || [self cl_isGzippedData])
    {
        return self;
    }
...
- (NSData *)cl_gzippedData
{
    return [self cl_gzippedDataWithCompressionLevel:-1.0f];
}

- (NSData *)cl_gunzippedData
{
    if (self.length == 0 || ![self cl_isGzippedData])
    {
        return self;
